...
def inception_resnet_v2(input_shape, classes, model_fpath):
    ...
    x = inception_resnet_A(x,name='moduleA')
    x = inception_resnet_B(x,name='moduleB')
    x = inception_resnet_C(x,name='moduleC')

    x = AveragePooling2D(2,padding='same',name='avgpool_2')(x)
    x = Dropout(0.25)(x)
    x = Flatten()(x)
...
def my_model_inception(input_shape, classes):
    ...
    layer = inception_resnet_A(visible, name='moduleA')
...
def inceptionV4(input_shape, classes):
    ...
    layer = AveragePooling2D((2, 2))(layer)
    layer = Conv2D(64, (1,1), padding='same', activation='relu')(layer) 
    layer = Conv2D(64, (3,3), padding='same', activation='relu')(layer) 
    # add inception-resnet block 1
    layer = inception_module_A(visible, 64, 32, 64, 32, 64, 64, 32, resnet=True)
    layer = AveragePooling2D((2, 2))(layer)
    layer = Dropout(0.25)(layer)
