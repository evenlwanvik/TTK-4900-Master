...
def plot_training_data():
    ...
    for i in range(1500, 1700):
        ...
        for j, fpath in enumerate((dirpath+'ssl_train.npz', dirpath+'uvel_train.npz', dirpath+'vvel_train.npz')):
            ...
            #data[j] = cv2.resize(data[j], dsize=(winH, winW), interpolation=cv2.INTER_CUBIC) # Resize to 
        ...
        ax.pcolormesh(data[0].T, cmap='rainbow')

        n=-1
        color_array = np.sqrt(((data[1]-n)/2)**2 + ((data[2]-n)/2)**2)
        ax.quiver(data[1].T, data[2].T, color_array, scale=2) # Plot vector field
