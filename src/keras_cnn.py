...
def test_model(nc_fpath='C:/Master/data/cmems_data/global_10km/2016/phys_noland_2016_001.nc', model_fpath=model_fpath, meastype=meastype):
    ...
    for (x, y, (lonIdxs, latIdxs)) in sliding_window(ssl, stepSize=stepSize, windowSize=dSize):
        ...
        #uvel_scaled_wind = np.array([[phase_scaled[i,j] for j in latIdxs] for i in lonIdxs])
        vvel_wind = np.array([[vvel[i,j] for j in latIdxs] for i in lonIdxs])
        #vvel_scaled_wind = np.array([[phase_scaled[i,j] for j in latIdxs] for i in lonIdxs])

        # Add dimensions for CNN
        ssl_cnn_window   = np.expand_dims(np.expand_dims(ssl_scaled_wind, 2), 0) # (1, 30, 18, 1)
        phase_cnn_window = np.expand_dims(np.expand_dims(phase_scaled_wind, 2), 0) # (1, 30, 18, 1)
        #uvel_cnn_window = np.expand_dims(np.expand_dims(uvel_scaled_wind, 2), 0)
        #vvel_cnn_window = np.expand_dims(np.expand_dims(vvel_scaled_wind, 2), 0)
