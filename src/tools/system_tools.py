...
def find_labeled(labeled, ctr):
    ...
    Start from center index and spiral outwards until we find first of 
    a connected and labeled OW binary cluster """
    nx, ny = labeled.shape
    x0, y0 = ctr
...
def expand_cluster(OW_eddies_box, box, xy, edge, label):
    """ Expand the edge dimensions of boundary
    If we make it through the column/row, we don't have any more OW masks in this 
    dimensions and we return True"""
    ...
    # Search through the edge for OW cells
    for i in range(n):
...
def include_full_cluster(labeled_array, box, label):
    """ Expand prediction untill full labeled eddy cluster is included
    ...
    # Checking through all edge dimensions
...
def get_min(data, b):
    """ Returns minimum value (could be OW value) and index of predicted box """
...
def box2coords(x, y, boxes):
    """ Convert box indeces to polar stereo proj coordinates """
    boxcoords = []
...
def set_eddy_census(cellclusters, ctrIdxs, boxes, minOW, lon, lat, xc=[], yc=[], uvel=None, vvel=None, res=0.083, meastype='SAT'):
    """ Set cencus (size, position, etc.) for eddy """
...
def deriv1_central_diff_2D(a,x,y):
    ...
    # centered central differences. The variable a is a 2D field.   
    nx,ny = a.shape
    dadx = np.zeros((nx,ny))                            
    dady = np.zeros((nx,ny))
...
def calc_OW(lon,lat,uvel,vvel,OW_start):  
    """ Okubo-Weiss parameter for 2D ocean current field. OW threshold (<=0) decides 
    what cells dominated by vorticity are considered as belonging to an eddy. Cells
    below threshold are returned as either cyclone mask or anti-cyclone mask depending
    on if the cell has positive or negative vorticity.
    
    Args:
        lon (float): longitude vector
        lat (float): latitude vector
        uvel (float): eastward velocity field
        vvel (float): northward velocity field
        OW_start (float): OW threshold
    Returns:
        OW (float): OW field
        vorticity (float): vorticity field
        cyc_mask (float): binary mask of cyclone field
       cyc_mask (float): binary mask of anti-cyclone field        
    """
    ########################################################################
...
def rotate_vector(u_east, v_north, lons, lon_displacement):
    """ Rotates vectors according to longitude and longitudinal displacement"""
...
def interp2d_masked(a, x, y, m, kind='linear'):
    ...
    in: Array, x, y, upsample factor (m) """
...
def find_nearest(array, value):
    """ Return array value closest to value """
